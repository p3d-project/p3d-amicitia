...
namespace Amicitia.ResourceWrappers
{
    ...
    internal interface IResourceWrapper : INotifyPropertyChanged
    {
        ...
        bool ExportToFolder(string folderPath);
    ...
    public abstract partial class ResourceWrapper<TResource> : TreeNode, IResourceWrapper, IDisposable
    {
        ...
        /// <summary>
        /// Exports this node's own file (using its native file type and extension) into the given folder.
        /// Used by the recursive "Export All" feature. Returns false if this node has nothing to export.
        /// </summary>
        public virtual bool ExportToFolder(string folderPath)
        {
            if (mFileExportActions == null || !mFileExportActions.ContainsKey(FileType))
                return false;

            var info = SupportedFileManager.GetSupportedFileInfo(FileType);
            var fileName = GetExportFileName(info);
            var fullPath = Path.Combine(folderPath, fileName);

            mFileExportActions[FileType].Invoke(Resource, fullPath);
            return true;
        }

        /// <summary>
        /// Builds a safe file name for this node, adding the type's default extension
        /// if the node's text doesn't already have one.
        /// </summary>
        internal string GetExportFileName(SupportedFileInfo info)
        {
            var name = Text.Trim();
            ...
            if (info.Extensions.Length > 0 && info.Extensions[0] != ".*")
            {
                var hasExtension = info.Extensions.Any(ext => name.EndsWith(ext, StringComparison.InvariantCultureIgnoreCase));
                if (!hasExtension)
                    name += info.Extensions[0];
            }
